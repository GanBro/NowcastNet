...
class DGMRTrainer(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        ...
        # 分离生成器和判别器的输出
        for output in outputs:
            if "log_dict" in output:
                log_dict = output["log_dict"]

                # 生成器的输出
                if "G/ G loss" in log_dict:
                    G_loss.append(log_dict["G/ G loss"])
                    G_score.append(log_dict["G/ D loss"])

                # 判别器的输出
                if "D/ D loss" in log_dict:
                    true_D_loss.append(log_dict["D/ true D loss"])
                    fake_D_loss.append(log_dict["D/ fake D loss"])
                    D_loss.append(log_dict["D/ D loss"])

        # 如果有数据,计算平均值并记录
        if len(G_loss) > 0:
            G_loss_mean = torch.mean(torch.stack(G_loss))
            G_score_mean = torch.mean(torch.stack(G_score))
            self.logger.experiment.add_scalar("G/ G loss", G_loss_mean, global_step=self.current_epoch)
            self.logger.experiment.add_scalar("G/ D loss", G_score_mean, global_step=self.current_epoch)

        if len(D_loss) > 0:
            true_D_loss_mean = torch.mean(torch.stack(true_D_loss))
            fake_D_loss_mean = torch.mean(torch.stack(fake_D_loss))
            D_loss_mean = torch.mean(torch.stack(D_loss))
            self.logger.experiment.add_scalar("D/ true D loss", true_D_loss_mean, global_step=self.current_epoch)
            self.logger.experiment.add_scalar("D/ fake D loss", fake_D_loss_mean, global_step=self.current_epoch)
            self.logger.experiment.add_scalar("D/ D loss", D_loss_mean, global_step=self.current_epoch)
